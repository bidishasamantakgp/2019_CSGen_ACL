...
class BaseModel(object):
  ...
  def __init__(self,
    ...
    if hparams.get_embeddings == True:
      self.infer_logits, _, self.final_context_state, self.sample_id, self.emb_input, self.encoder_outputs, self.decoder_outputs, self.decoder_inp = res
    ...
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
      ...
      self.infer_logits, _, self.final_context_state, self.sample_id, _, _ = res
      self.sample_words = reverse_target_vocab_table.lookup(
          tf.to_int64(self.sample_id))
   '''
    #if hparams.get_embeddings == True:
    #  self.infer_logits, _, self.final_context_state, self.sample_id, self.encoder_outputs, self.decoder_outputs = res
  ...
  def init_embeddings(self, hparams, scope):
    """Init embeddings."""
    print("self.src_vocab_size", self.src_vocab_size)	   
  ...
  def train(self, sess):
    ...
    return sess.run([self.update,
                     ...
                     self.word_count,
                     self.batch_size])
  def getembeddings(self, sess):
    #assert self.mode == tf.contrib.learn.ModeKeys.TRAIN
    #print("batch_size", self.batch_size.eval(session=sess))
    return sess.run([self.encoder_outputs, self.decoder_outputs, self.emb_input, self.decoder_inp, self.final_context_state.alignment_history.stack()])
  ...
  def build_graph(self, hparams, scope=None):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=dtype):
      # Encoder
      encoder_outputs, encoder_state, _emb_input = self._build_encoder(hparams)

      ## Decoder
      logits, sample_id, final_context_state, decoder_outputs, decoder_emb_inp = self._build_decoder(
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        loss = None
      if hparams.get_embeddings == True:
        return logits, loss, final_context_state, sample_id, _emb_input, encoder_outputs, decoder_outputs, decoder_emb_inp
      else: 
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams):
    ...
    return logits, sample_id, final_context_state, outputs, decoder_emb_inp
...
class Model(BaseModel):
  ...
  def _build_encoder(self, hparams):
    ...
    with tf.variable_scope("encoder") as scope:
      ...
      print("Embed input", encoder_emb_inp)
      print("Encoder embeding",  self.embedding_encoder)
